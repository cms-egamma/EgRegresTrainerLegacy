...
//  gROOT->ProcessLine("res.makeHists({treeEleStep3,treeEleStep4},\"\",\"mc.energy>0 && sc.rawEnergy>0 && ssFrac.sigmaIEtaIEta>0 && mc.dR<0.1 && ele.et>0 && eventnr%5>=3\",\"mc.pt\",\"sc.seedEta\",etBins,etaBins2p5)");
  ...
  gROOT->ProcessLine("res.printFits({1,2,3},\"../plots/2016UL_ConstParameters/DCB_Eta_\")");
 }
 ...
 if(puPlot){
  ...
  gROOT->ProcessLine("res.printFits({1,2,3},\"../plots/2016UL_ConstParameters/DCB_PU_EB_\")");
  ...
  gROOT->ProcessLine("res.printFits({1,2,3},\"../plots/2016UL_ConstParameters/DCB_PU_EE_\")");
 }
 ...
 if(ePlot){
  ...
  gROOT->ProcessLine("res.printFits({1,2,3},\"../plots/2016UL_ConstParameters/DCB_Et_EB_\")");
  ...
  gROOT->ProcessLine("res.printFits({1,2,3},\"../plots/2016UL_ConstParameters/DCB_Et_EE_\")");
 }
}
